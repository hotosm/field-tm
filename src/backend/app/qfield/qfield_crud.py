...
async def create_qfield_project(
    ...
    try:
        async with db.cursor() as cur:
            await cur.execute(
                f"DROP TABLE IF EXISTS temp_task_boundaries_{project.id} CASCADE;"
            )
            await db.commit()
            log.debug(f"Cleaned up temp task boundaries table for project {project.id}")
    except Exception as e:
        log.warning(
            f"Failed to clean up temp task boundaries table for project {project.id}: {e}"
        )
