...
- add project deletion button for project manager
- add project manager username to details page
- **backend**: add qfieldcloud collaborator addition to API as well as htmx
- add qfield admin page to add users to project as qfc collaborators
- **backend**: fix ##3002, add osm_username to odk survey
- **area-splitter**: Use Straight Skeleton algorithm to split tasks. (#2999)
...
- **qfield**: tasks layers not adding to qfield project layer tree
- **qfield**: ensure the task layer is added to the qfield project
- **qgis**: add mapcanvas element into generated qgz file from xlsform2qgis
- **qfield**: strip dangling temp files before qgis project packaging
- **backend**: replace usage of http response integers with litestar enums
- **backend**: replace qfield user creation wrapper with upstream contributed api
- **backend**: pre-commit hooks + small bugs in project creation
- tweaks to logic to pass pre-commit checks
- working qfield setup, header styling, deploy + test config
- qfield project setup working with latest xlsformconverter + garage s3
- **backend**: get existing appuser token without re-init
- **backend**: usage of odk central url for appuser qrcode
- implement hotosm/ui css and design tokens correctly
- add tests that confirm fix #2868
- **osm-fieldwork**: improve extraction of languages from xlsforms
- small fix for cla sign test (#3023)
- **backend**: prompt user for additiona media, fix #2342
- pre-commit issues
- **backend**: access odk central locally, fix #2907
- small fix to test cla (#3022)
- usability and clarity of ui
- **backend**: strip odk central url v1 prefix
- **backend**: project creation api fixes
- **backend**: replace osm_fieldwork odk wrappers with pyodk, fixes #2210
- **area-splitter**: replace osm_rawdata dependency with raw_data_api_py fixes #2826
- **frontend**: update Sentry to React 19 compatible API (#3007)
...
- **backend**: move /htmxprojects --> /projects, add auth hook to save user in db
- **backend**: remove old user routes apis (unused)
- **backend**: replace usage of pyproj pkg with PostGIS query
- **backend**: remove geojson package as it's not well maintained (just use dict type hints)
- **backend**: remove usage of shapely, defer to PostGIS implementations
- run pre-commit hooks
- clean up unused API routes, keeping the essentials
- updates to qfield project creation wrapper
- fmtm references --> ftm or fieldtm
- **backend**: remove flatgeobuf --> geojson conversion entirely
- fix all pre-commit issues
- **backend**: add new pre-commit hooks and run + fix issues
- use a single endpoint for JSON API based project creation
- restructure project creation flow
- cleanup project details page
- update to odk central, various refactors and fixes #2960
- break large setup steps file into smaller chunks
- update landing page to reflect previous design
- remove redundant code, fix pre-commit
- finalise odk projects, geojson-aoi-parser pkg, official pyodk only
- odk central enums fix #2978, workflow in collect only fixes #2963
- fix osm-fieldwork test + run pre-commit hooks
- remove react frontend
- large refactor to have htmx + api token json in parallel + landing page
- wip project creation process
- **area-splitter**: configure a more modular area splitter
- remove features ref #2945 (#2995)
- remove backend code that is no longer used
- further code removal, refactor, and fixes
- wip removal of redundant code
- remove additional refs to minio
- remove redundant env vars
- remove tasks + org router, update db models
- **backend**: yxf library to most recent release (#2982)
- **manageProject**: improve instructions section on form update (#2910)
